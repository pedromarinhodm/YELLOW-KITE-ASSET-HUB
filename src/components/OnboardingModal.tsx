...
export function OnboardingModal({
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Only station equipment for allocations
  ...
  const handleSelectAll = () => {
    if (selectedEquipments.length === availableEquipments.length) {
      setSelectedEquipments([]);
    } else {
      setSelectedEquipments(availableEquipments.map(e => e.id));
    }
  };
            ...
            <div className="flex items-center justify-between">
              ...
              {availableEquipments.length > 1 && (
                <Button
                  ...
                  className="text-xs"
                >
                  {selectedEquipments.length === availableEquipments.length
                    ? 'Desmarcar todos'
                    : 'Selecionar todos'}
                </Button>
              )}
